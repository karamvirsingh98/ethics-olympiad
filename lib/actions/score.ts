...
export const SUBMIT_SCORES_ACTION = judgeActionClient
  ...
  .action(async ({ ctx, parsedInput }) => {
    ...
    // Pull olympiadId + teams in one select so we can revalidate and validate
    // teams without a second round-trip.
    const [event] = await db
      .select({
        olympiadId: eventsTable.olympiadId,
        teams: eventsTable.teams,
      })
      .from(eventsTable)
    ...
    revalidatePath(`/manager/olympiads/${event.olympiadId}/${eventId}`);
