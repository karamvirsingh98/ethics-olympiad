...
The `backend` is deployed on Heroku as a docker container. To build and push a new image, run the `deploy` script found at `/backend/deploy.sh`. The `manager` and `events` apps are hosted on vercel, and can be deployed by running their specific deployment scripts found in the `/deploy` folder.
